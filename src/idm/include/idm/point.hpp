...
/* 计算点到直线的距离 */
template <class T>
T distance(const point<T> &p0, const point<T> &p1, const point<T> &p2)
{
	point<T> pp = p1 - p2;
	return fabs(pp.x * p0.y - pp.y * p0.x) / sqrt(p0.x * p0.x + p0.y * p0.y);
}
...
/*点的 << 运算符的重载*/
template <class T, class A>
std::ostream &operator<<(std::ostream &os, orientedpoint<T, A> &p)
{
	os << "(" << p.x << " " << p.y << " " << p.theta << ")";
	return os;
}
...
struct TransPoint
{
	OrientedPoint o; // （0，0，0）-> o
	TransPoint(const OrientedPoint &o_) : o(o_) {}
	void operator()(Point &p)
	{
		double x = p.x, y = p.y;
		p.x = o.x + cos(o.theta) * x - sin(o.theta) * y;
		p.y = o.y + sin(o.theta) * x + cos(o.theta) * y;
	}
};
